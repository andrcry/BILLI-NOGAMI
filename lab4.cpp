...
#include <limits>     // Для очищення буфера
...
// --- [НОВЕ] ФУНКЦІЯ РЕДАГУВАННЯ КНИГИ ---
void adminEditBook(vector<Book>& db) {
    int id;
    displayAllBooks(db); // Показуємо список, щоб знати ID
    cout << "\n[ADMIN] Введіть ID книги для редагування: ";
    cin >> id;
    ...
    for (auto& book : db) {
        if (book.id == id) {
            found = true;
            cout << "\n--- Редагування книги ID: " << book.id << " ---" << endl;
            cout << "Поточні дані: " << book.title << ", " << book.price << " грн" << endl;

            // Логіка зміни ціни
            double newPrice;
            cout << "Введіть нову ціну (або -1, щоб залишити стару): ";
            cin >> newPrice;
            if (newPrice != -1) book.price = newPrice;

            // Логіка зміни назви
            string newTitle;
            cout << "Введіть нову назву (або '-', щоб залишити стару): ";
            cin >> newTitle;
            if (newTitle != "-") book.title = newTitle;

            // Логіка зміни автора
            string newAuthor;
            cout << "Введіть нового автора (або '-', щоб залишити старого): ";
            cin >> newAuthor;
            if (newAuthor != "-") book.author = newAuthor;

            cout << "[Успіх] Інформацію оновлено!" << endl;
            break;
        }
    }

    if (!found) {
        cout << "[Помилка] Книгу з таким ID не знайдено." << endl;
    }
}
...
void runCatalogModule(vector<Book>& db, vector<Book>& cart, const User* user) {
    ...
    do {
        ...
        if (user->role == "admin") {
            ...
            cout << "10. [ADMIN] Редагувати книгу" << endl; // Додано новий пункт
            cout << "-----------------------" << endl;
        }
        ...
        case 9:
            ...
            break;
        case 10: // Обробка нового пункту
            if (user->role == "admin") adminEditBook(db);
            else cout << "Доступ заборонено." << endl;
            break;
...
bool processPaymentLogic(const string& method, double amount) {
    ...
    if (method == "card") {
        ...
        for (char c : card) {
            if (!isdigit(c)) {
                cout << "Помилка: номер картки має містити лише цифри!" << endl;
                return false;
            }
        }
    }
