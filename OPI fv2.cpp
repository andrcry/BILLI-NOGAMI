...
void addToCart(const vector<Book>& db, vector<Book>& cart) {
    ...
    if (cin.fail()) {
        cin.clear();            // 1. Знімаємо прапор помилки cin
        cin.ignore(1000, '\n'); // 2. Видаляємо "сміття" (букву 'e') з буфера
        cout << "[Помилка] ID має бути числом! Спробуйте ще раз." << endl;
        return;                 // 3. Виходимо з функції, щоб не ламати програму далі
    }
